...
import EditCategoryModal from '@/components/expense/EditCategoryModal';
import ViewCategoryModal from '@/components/expense/ViewCategoryModal';
...
export default function ExpenseCategoriesPage() {
  ...
  const fetcher = async (url) => {
    try {
      const res = await axios.get(`${API_URL}/get-expense-type-list`, {
  ...
  // Try get-expense-type-list endpoint
  const { data: categoryRes, isLoading, mutate } = useSWR(
    token ? `${API_URL}/get-expense-type-list` : null,
    fetcher,
    {
  ...
  return (
    <div className="space-y-5 text-black">
      {/* Tab Header */}
      <ExpenseTabNav activeTab="categories" />
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            ...
            placeholder="Search expense category by name, type, description..."
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-neutral-200/80 overflow-hidden">
        {isLoading ? (
          <div className="py-16 text-center text-neutral-400">
            ...
            <p className="text-xs">Loading expense categories...</p>
          </div>
          ...
          <>
            ...
            <div className="block sm:hidden divide-y divide-neutral-100">
              {filteredCategories.map((category) => {
                ...
                const transCat = category.transaction_category || 'Expense';
