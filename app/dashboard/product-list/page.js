                ...
                filteredProducts.map((product) => (
                  <tr key={product.id} className="hover:bg-neutral-50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-sm font-medium text-neutral-900 text-right">
                      {product.purchase_price ? `৳${parseFloat(product.purchase_price).toLocaleString()}` : '-'}
                    </td>
                    <td className="py-4 px-6 text-sm font-medium text-neutral-900 text-right">
                       {product.retails_price || product.sell_price ? `৳${parseFloat(product.retails_price || product.sell_price).toLocaleString()}` : '-'}
                    </td>
                  </tr>
                ))
              ) : (
