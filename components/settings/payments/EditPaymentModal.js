...
export default function EditPaymentModal({ isOpen, onClose, onSuccess, method }) {
  ...
  useEffect(() => {
    if (method && isOpen) {
      ...
      if (method.icon_image) {
        setPreview(method.icon_image.startsWith('http') ? method.icon_image : `${process.env.NEXT_PUBLIC_BASE_URL || API_URL?.replace('/api', '')}/${method.icon_image}`);
      } else {
        setPreview('');
      }
      setImageFile(null);
    }
  }, [method, isOpen, API_URL]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    let uploadedImagePath = formData.icon_image;
    ...
    try {
      ...
      const payload = {
        ...formData,
        icon_image: uploadedImagePath,
      };

      const res = await axios.post(`${API_URL}/payment-type-update`, payload, {
        headers: { Authorization: `Bearer ${token}` },
      });

      if (res.data?.success || res.status === 200) {
        toast.success(res.data?.message || 'Payment method updated successfully!');
        onSuccess?.();
        onClose();
      } else {
        toast.error(res.data?.message || 'Failed to update payment method.');
      }
    } catch (err) {
      console.error('Update payment error:', err);
      toast.error(err.response?.data?.message || 'Error updating payment method.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-5 py-4 border-b border-neutral-100 flex items-center justify-between shrink-0 bg-white">
          <h2 className="text-lg font-bold text-neutral-800">Edit Payment Method</h2>
          <button
            onClick={onClose}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                className="w-full px-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-sm text-neutral-800 focus:outline-none focus:ring-2 focus:ring-black/5 focus:border-black transition-all"
              >
