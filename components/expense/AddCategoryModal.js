  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        transaction_category: formData.transaction_category || 'Expense',
      ...
      if (res.data?.success || res.status === 200) {
        toast.success(res.data?.message || 'Expense category created successfully!');
        setFormData({ expense_name: '', transaction_category: 'Expense', expense_description: '' });
        onSuccess?.();
        onClose();
      } else {
      ...
      toast.error(err.response?.data?.message || 'Failed to create expense category.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-xl border border-neutral-200 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between bg-neutral-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-neutral-900">Add Expense Category</h3>
              <p className="text-xs text-neutral-500">Create a new category for grouping expenses</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4 overflow-y-auto">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              placeholder="e.g. Office Supplies, Snacks, Rent..."
              className="w-full px-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-sm text-neutral-900 placeholder-neutral-400 outline-none focus:ring-2 focus:ring-black transition-all"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea
              ...
              className="w-full px-3.5 py-2.5 bg-neutral-50 border border-neutral-200 rounded-xl text-sm text-neutral-900 placeholder-neutral-400 outline-none focus:ring-2 focus:ring-black transition-all resize-none"
            />
          </div>
          ...
          <div className="pt-2 flex items-center gap-3">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 py-2.5 bg-black hover:bg-neutral-800 text-white rounded-xl text-xs font-semibold shadow-sm transition-all cursor-pointer flex items-center justify-center gap-2 disabled:opacity-50"
            >
              {loading ? (
                <>
