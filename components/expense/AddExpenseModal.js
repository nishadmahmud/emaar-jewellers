...
import { X, Loader2, CheckCircle2, ChevronDown, Check, DollarSign } from 'lucide-react';
...
export default function AddExpenseModal({ open, onClose, onSuccess }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Error occurred while saving expense.');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-md overflow-hidden border border-neutral-100 text-black flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-neutral-100 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-neutral-500">Record a new operating expense</p>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-4">
          {loadingInitial ? (
            <div className="py-12 text-center text-neutral-400">
              ...
              <p className="text-xs font-medium">Loading expense categories...</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <CustomSelect
                ...
                options={categories.map((c) => ({ id: c.id, name: c.name || c.catogory_name }))}
