...
export default function EditCategoryModal({ open, category, onClose, onSuccess }) {
  ...
  useEffect(() => {
    if (category) {
      setFormData({
        expense_name: category.expense_name || category.name || category.catogory_name || '',
        transaction_category: category.transaction_category || 'Expense',
        expense_description: category.expense_description || category.description || '',
      });
    }
  }, [category]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/update-expense-type/${category.id}`, payload, {
      ...
      toast.error(err.response?.data?.message || 'Failed to update expense category.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl max-w-md w-full overflow-hidden shadow-xl border border-neutral-200 flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-4 border-b border-neutral-100 flex items-center justify-between bg-neutral-50/50">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-bold text-base text-neutral-900">Edit Expense Category</h3>
              <p className="text-xs text-neutral-500">Update category #{category.id}</p>
            </div>
          </div>
          <button
            type="button"
